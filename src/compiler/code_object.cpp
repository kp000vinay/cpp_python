...
int CodeObject::add_const(const PyConstant& value) {
...
int CodeObject::add_name(const std::string& name) {
...
int CodeObject::add_varname(const std::string& name) {
...
int CodeObject::find_varname(const std::string& name) const {
    auto it = std::find(co_varnames.begin(), co_varnames.end(), name);
    if (it != co_varnames.end()) {
        return static_cast<int>(it - co_varnames.begin());
    }
...
int CodeObject::add_freevar(const std::string& name) {
...
int CodeObject::add_cellvar(const std::string& name) {
...
void CodeObject::emit(Opcode op, int arg, int lineno) {
    Instruction instr(op, arg, lineno);
    instr.offset = current_offset();
    instructions.push_back(instr);
}

int CodeObject::current_offset() const {
    // Each instruction is 2 bytes in Python 3.6+
    // With EXTENDED_ARG, it can be more
    int offset = 0;
    for (const auto& instr : instructions) {
        offset += 2;  // Base instruction size
        // Add EXTENDED_ARG bytes if needed
        if (instr.arg > 0xFF) {
            offset += 2;  // One EXTENDED_ARG
            if (instr.arg > 0xFFFF) {
                offset += 2;  // Two EXTENDED_ARGs
                if (instr.arg > 0xFFFFFF) {
                    offset += 2;  // Three EXTENDED_ARGs
                }
            }
        }
    }
...
void CodeObject::calculate_stacksize() {
    // Simple stack depth calculation
    // A more accurate implementation would trace all paths
    int depth = 0;
    int max_depth = 0;
    ...
    for (const auto& instr : instructions) {
        ...
        // Ensure depth doesn't go negative (would indicate a bug)
        if (depth < 0) {
            depth = 0;
        }
    }
    
    co_stacksize = max_depth + 1;  // Add some buffer
}
...
std::string CodeObject::disassemble() const {
    ...
    for (const auto& instr : instructions) {
        ...
        oss << "\n";
        offset += 2;  // Simplified offset calculation
    }
